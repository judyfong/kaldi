...
if [ $stage -le -1 ]; then
    ...
    local/run_segmentation.sh data/all malromur/lang malromur <date>
...
if [ $stage -le 1 ]; then
    ...
    comm -13 <(cut -f1 ~/data/althingi/pronDict_LM/pron_dict_20161005_plusAlthingi.txt | sort) \
	 <(sort ~/data/althingi/pronDict_LM/scrapedAlthingiTexts_clean_words3.txt) > ~/data/althingi/scrapedAlthingiTexts_clean_words3.txt
    ...
    frob=~/data/althingi/pronDict_LM/pron_dict_20161123.txt
    local/prep_lang.sh \
        $frob             \
        data/train         \
        data/local/dict     \
