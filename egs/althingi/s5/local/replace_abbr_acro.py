...
text = re.sub("a\.m\.k\."," að minnsta kosti ",text)
text = re.sub("dr\."," doktor ",text)
text = re.sub("e\.t\.v\."," ef til vill ",text)
text = re.sub("frh\."," framhald ",text)
text = re.sub("m\.a\.s\."," meira að segja ",text)
text = re.sub("m\.a\."," meðal annars ",text)
text = re.sub("nk\."," næstkomandi ",text)
text = re.sub("nr\."," númer ",text)
text = re.sub("o\.s\.frv\."," og svo framvegis ",text)
text = re.sub("sbr\."," samanber ",text)
text = re.sub("skv\."," samkvæmt ",text)
text = re.sub("t\.d\."," til dæmis ",text)
text = re.sub("u\.þ\.b\."," um það bil ",text)
