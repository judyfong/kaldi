...
cat $input_dir/spk2utt | perl -e '
  ($text_in, $wav_in, $text_out, $wav_out, $sox, $sil_wav) = @ARGV;
  ...
  while (<WI>) {
    ...
    @col = split;
    @col >= 2 || die "Error: bad line $_\n";
    $wav{$col[0]} = " \"| @col[1 .. 11]\""; # Changed from col[4] since I have mp3 files which have to be converted to wav and resampled
  }
