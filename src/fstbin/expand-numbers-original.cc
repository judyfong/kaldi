...
int main(int argc, char *argv[])
{
  try {
    ...
    const char *usage =
        ...
        "\n"
        "<language-model> may be either in ARPA or FST format\n";
    ...
    po.Register("lm-input-type", &lm_input_type,
                "Language model format. One of: \"arpa\", \"fst\"");
    ...
    if (word_symbol_table_filename.empty() && lm_input_type == "arpa") {
      KALDI_ERR << "LM in ARPA format requires word-symbol-table to be set";
    }
    ...
    for (; !transcript_reader.Done(); transcript_reader.Next(), n_done++) {
      ...
      // // Add eos and bos markers to the transcript if we got the LM from ARPA fmt
      // fst::StdArc::StateId bos = transcript_fst.AddState();
      // transcript_fst.SetStart(bos);
      // transcript_fst.AddArc(bos, const Arc &arc)
      // fst::StdArc::StateId eos = transcript_fst.AddState();
      // transcript_fst.SetFinal(eos);
