...
abbr_list=$root_text_norm_listdir/abbreviation_list.txt
scraped_text_dir=$root_raw_text
...
sbatch --get-user-env --mem 8G \
       --output=${out}/log/preprocessing_trainingdata_cs.log \
       --wrap="srun python punctuator/local/preprocessing_trainingdata_cs.py ${cleaned}/punctuation_text.train.txt ${out}/althingi.train.txt"

python punctuator/local/preprocessing_trainingdata_cs.py \
       ${cleaned}/punctuation_text.dev.txt \
       ${out}/althingi.dev.txt

python punctuator/local/preprocessing_trainingdata_cs.py \
       ${cleaned}/punctuation_text.test.txt \
       ${out}/althingi.test.txt
...
punctuator/local/clean_AlthingiData_pause.sh \
  ...
  ${cleaned}/texts_pause_clean_for_punct_restoring.txt

echo "Preprocess the data"
sbatch --get-user-env --mem 8G --time 0-04:00 --nodelist=terra \
       --wrap="srun python punctuator/local/preprocessing_pause_data.py ${cleaned}/texts_pause_clean_for_punct_restoring.txt ${out_2nd_stage}/althingi.train_Sept2017_pause_simpleExp.txt"

echo "Make the Sept 2017 data pause annotated"
utils/slurm.pl ${out_2nd_stage}/log/make_pause_annotated.log punctuator/local/make_pause_annotated.sh
