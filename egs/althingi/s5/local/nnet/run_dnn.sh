...
if [ $stage -le 0 ]; then
  ...
  steps/nnet/make_fmllr_feats.sh --nj 10 --cmd "$train_cmd" \
     --transform-dir $gmmdir/decode_tg_bd_dev \
     $dir data/dev $gmmdir $dir/log $dir/data || exit 1
  # eval
  dir=$data_fmllr/eval
  steps/nnet/make_fmllr_feats.sh --nj 10 --cmd "$train_cmd" \
     --transform-dir $gmmdir/decode_tg_bd_eval \
     $dir data/eval $gmmdir $dir/log $dir/data || exit 1
  # train
  dir=$data_fmllr/train
  steps/nnet/make_fmllr_feats.sh --nj 12 --cmd "$train_cmd" \
     --transform-dir ${gmmdir}_ali_bd \
...
if [ $stage -le 2 ]; then
  ...
  $cuda_cmd $dir/log/train_nnet.log \
    ...
    $data_fmllr/train_tr90 $data_fmllr/train_cv10 data/lang_bd $ali $ali $dir || exit 1;
  # Decode (reuse HCLG graph)
  steps/nnet/decode.sh --nj 10 --cmd "$decode_cmd" --config conf/decode_dnn.config --acwt 0.1 \
    $gmmdir/graph_tg_bd $data_fmllr/dev $dir/decode_tg_bd_dev || exit 1;
  steps/nnet/decode.sh --nj 10 --cmd "$decode_cmd" --config conf/decode_dnn.config --acwt 0.1 \
    $gmmdir/graph_tg_bd $data_fmllr/eval $dir/decode_tg_bd_eval || exit 1;
fi
...
if [ $stage -le 3 ]; then
  ...
  steps/nnet/align.sh --nj 100 --cmd "$train_cmd" \
    $data_fmllr/train data/lang_bd $srcdir ${srcdir}_ali || exit 1;
  steps/nnet/make_denlats.sh --nj 100 --cmd "$decode_cmd" --config conf/decode_dnn.config --acwt $acwt \
    $data_fmllr/train data/lang_bd $srcdir ${srcdir}_denlats || exit 1;
fi

if [ $stage -le 4 ]; then
  # Re-train the DNN by 1 iteration of sMBR 
  steps/nnet/train_mpe.sh --cmd "$cuda_cmd" --num-iters 5 --acwt $acwt --do-smbr true \
    $data_fmllr/train data/lang_bd $srcdir ${srcdir}_ali ${srcdir}_denlats $dir || exit 1
  # Decode (reuse HCLG graph)
  for ITER in 5 4 3 1; do
    steps/nnet/decode.sh --nj 10 --cmd "$decode_cmd" --config conf/decode_dnn.config \
      --nnet $dir/${ITER}.nnet --acwt $acwt \
      $gmmdir/graph_tg_bd $data_fmllr/dev $dir/decode_tg_bd_dev_it${ITER} || exit 1;
    steps/nnet/decode.sh --nj 10 --cmd "$decode_cmd" --config conf/decode_dnn.config \
      --nnet $dir/${ITER}.nnet --acwt $acwt \
      $gmmdir/graph_tg_bd $data_fmllr/eval $dir/decode_tg_bd_eval_it${ITER} || exit 1;
  done 
fi
