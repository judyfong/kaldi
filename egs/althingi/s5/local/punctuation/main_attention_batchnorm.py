...
def attention2(inputs,context,projected_context):
    ...
    attention = Dense(TIME_STEPS)(attention)
...
def createModel(num_hidden, learning_rate):
    ...
    context = Bidirectional(GRU(num_hidden, activation='tanh', recurrent_activation='hard_sigmoid', use_bias=False, kernel_initializer='glorot_uniform', return_sequences=True))(e)
    ...
    grulayer_uni = GRU(num_hidden, activation='tanh', recurrent_activation='hard_sigmoid', use_bias=False, kernel_initializer='glorot_uniform', return_sequences=True)(projected_context)
    grulayer_uni = Dropout(DROP_RATE)(grulayer_uni)
    ...
    fusion_weights = BatchNormalization()(fusion_weights)
    fusion_weights = Activation('sigmoid')(fusion_weights)
    h = Add()([Multiply()([fusion_context,fusion_weights]), grulayer_uni])
    h = TimeDistributed(Dense(LABEL_DIM))(h)
    h = BatchNormalization()(h)
    y = Activation('softmax')(h)    

    model = Model(inputs=inputs, outputs=y)

    adam = optimizers.Adam(lr=learning_rate, clipnorm=1.1)
...
def trainModel(model, xTrain, yTrain, xVal, yVal, model_file, logdir, callbacks=None, verbose=False):
    sys.stderr.write("Training" + "\n")

    #From https://github.com/flomlo/ntm_keras/blob/master/testing_utils.py
    tensorboard = TensorBoard(log_dir=logdir, batch_size=MINIBATCH_SIZE, histogram_freq=1, write_grads=True, write_images=False) #, embeddings_freq=1, embeddings_layer_names='embedding', embeddings_metadata=model_path + '/logs' + 'metadata.tsv', embeddings_data=xTrain)
...
def testModel(model, xTest, yTest):
    score = model.evaluate(xTest, yTest, batch_size=MINIBATCH_SIZE)
    return score
...
if __name__ == "__main__":
    ...
    xTest, yTest = get_data(data.TEST_FILE,False)
    ...
    trainModel(model, xTrain, yTrain, xVal, yVal, model_file, logdir)

    score = testModel(model, xTest, yTest)
